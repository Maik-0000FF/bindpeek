            ...
            Flow {
                ...
                Repeater {
                    ...
                    delegate: ColumnLayout {
                        ...
                        // The shortcut column is as wide as the widest shortcut in
                        // this group and no wider.
                        //
                        // A fixed width is wrong in both directions at once: a
                        // group of single letters leaves two thirds of it empty
                        // and pushes every description away from its key, while a
                        // "CTRL+SHIFT+Delete" is cut off in the same panel. Each
                        // group answers for itself, which is where the eye reads
                        // down a column anyway.
                        //
                        // The longest string is the widest one because this column
                        // is set in a monospaced face; measured rather than
                        // counted all the same, so the number is in pixels and not
                        // in guessed character widths.
                        readonly property string widestShortcut: {
                            var widest = "";
                            var entries = groupCard.modelData.entries;
                            for (var i = 0; i < entries.length; ++i) {
                                var cell = panel.deeperInSections ? entries[i].key : entries[i].shortcut;
