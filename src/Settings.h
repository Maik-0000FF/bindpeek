...
//
// Source takes the headings the session itself uses: the section comment under
// mango, the submap under Hyprland, the mode under sway, the application under
// KDE. Modifiers ignores those and heads a group with the combination its
// shortcuts belong to, so the panel reads left to right by how many keys are
// still wanted: what fires now, then one modifier away, then two.
//
